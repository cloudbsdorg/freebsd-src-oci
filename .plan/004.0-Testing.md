...
- [8. Testing Strategy and Isolated Test Harnesses](#8-testing-strategy-and-isolated-test-harnesses)
  ...
  - [8.2 Unit Test Harness: `tests/usr.sbin/ocifbsd/ocifbsd_test.c`](#82-unit-test-harness-testsusrsbinocifbsd_testc)
...
**Execution authority for quality gates:** [021.0-Consolidated-Execution-Plan.md](./021.0-Consolidated-Execution-Plan.md) §8 (red–green–refactor, coverage, FreeBSD ATF/Kyua). Design chapters may show example harness code; **implemented** tests live under `tests/usr.sbin/ocifbsd/` and must follow FreeBSD ATF conventions.

**Honcho:** Record non-obvious test failures and lab constraints (freedev, Bastille, BE) so the next agent does not rediscover them.

**Lab deep-dive (caching + agent-readable regression):**  
[`.omo/drafts/freebsd-build-cache-and-regression.md`](../.omo/drafts/freebsd-build-cache-and-regression.md) — `WITH_CCACHE_BUILD`, meta/filemon, kyua `report-junit` / HTML / SQL, PTY vs `atf_check`, full regression artifact layout for agents.

### 8.1 Testing Philosophy

| Level | Environment | Purpose |
|-------|-------------|---------|
| Unit Tests | User-mode mock framework / ATF on FreeBSD | Test logic in isolation; **red then green** |
| Integration Tests | fredev host, bhyve VM, or disposable jail + BE/snapshot rollback | Test jail creation, networking, ZFS operations |
...
**COMPLETED work** requires red–green ATF for the claimed behavior. Do not mark lifecycle commands COMPLETED while `ocifbsd_test.c` remains disabled without a replacement suite.
